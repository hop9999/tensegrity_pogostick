function [t,x,p3_ar,p4_ar,f_ar,fr_ar,dl_ar,mom_ar,robot] = decor_ode_pogostick_stance(t_int,x_0,p3,robot)
    ...
    f_ar = zeros(4,length(t));
    fr_ar = zeros(3,length(t));
    ...
    for j = 1:length(t)
        ...
        p1 = CoM + r;
        p2 = CoM - r;
        [p4, f14, f13, f24, f23] = static_stance(p1,p2,p3,dl_ar(j),robot);
        mom_ar(:,j) = moment_massive_rod(theta_p,f14,f13,f24,f23,robot);
        f_ar(:,j) = [norm(f14); 
                      norm(f13); 
                      norm(f24); 
                      norm(f23)];
